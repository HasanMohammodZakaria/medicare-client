    ...
    return (
        <aside
            className="
                ...
                lg:h-full lg:overflow-x-visible lg:overflow-y-auto
            "
            style={{ backgroundColor: "var(--surface)", borderRight: "1px solid var(--border)", flexShrink: 0 }}
        >
            <Link href="/" className="hidden lg:flex items-center px-5 py-4.5 shrink-0" style={{ borderBottom: "1px solid var(--border)" }}>
                <span className="text-xl font-bold" style={{ fontFamily: "var(--font-heading)" }}>
                    <span style={{ color: "var(--primary)" }}>Medi</span>
                    <span style={{ color: "var(--text-primary)" }}>Nexa</span>
                </span>
            ...
            <div className="hidden lg:block px-5 pt-4 pb-2">
                <span className="text-xs font-semibold px-3 py-1 rounded-full"
                    style={{ backgroundColor: `color-mix(in srgb, ${ROLE_COLOR[role]} 15%, transparent)`, color: ROLE_COLOR[role] }}>
                    {role?.charAt(0).toUpperCase() + role?.slice(1)} Panel
                </span>
            </div>

            <nav className="flex flex-row gap-1 lg:flex-col lg:gap-0 lg:flex-1 lg:px-3 lg:py-2">
                {navItems.map(({ label, href, icon: Icon }) => {
                    const active = href === `/dashboard/${role}` ? pathname === href : pathname.startsWith(href);
                    return (
                        <Link key={href} href={href}
                            className="flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium whitespace-nowrap shrink-0 lg:gap-3 lg:px-3 lg:py-2.5 lg:text-sm lg:rounded-xl lg:mb-0.5 transition-all duration-150"
                            style={{
        ...
        <header className="h-16 flex items-center justify-between px-4 sm:px-6 shrink-0"
            ...
            <div className="flex items-center gap-2">
                ...
                <button className="p-2 rounded-xl relative transition-colors"
                    style={{ color: "var(--text-secondary)", backgroundColor: "var(--surface-secondary)" }}>
                    <MdNotifications size={18} />
                    <span className="absolute top-1.5 right-1.5 w-2 h-2 rounded-full" style={{ backgroundColor: "var(--danger)" }} />
                </button>
                ...
                <div className="relative">
                    <button onClick={() => setDropdownOpen((p) => !p)}
                        ...
                        {/* ✅ image সবসময় latest user.image দেখাবে */}
                        {user?.image ? (
                            <Image
                                src={user.image}
                                alt={user.name ?? "User"}
                                width={28} height={28}
                                className="rounded-full object-cover"
                            // ✅ browser cache bypass করবে না, কিন্তু session update হলে নতুন src আসবে
                            />
                        ) : (
                        ...
                        <span className="hidden sm:block text-sm font-medium max-w-27.5 truncate">{user?.name ?? "User"}</span>
                        <MdKeyboardArrowDown size={16} style={{ color: "var(--text-muted)", transform: dropdownOpen ? "rotate(180deg)" : "rotate(0)", transition: "transform 0.2s" }} />
                    </button>
...
export default function DashboardLayout({ children }) {
    ...
    // ✅ useSession() — reactive, profile update হলে automatically নতুন data আসবে
    ...
    useEffect(() => {
        ...
        if (!["patient", "doctor", "admin"].includes(role)) router.replace("/auth/login");
    ...
    return (
        <div className="h-screen flex flex-col lg:flex-row overflow-hidden" style={{ backgroundColor: "var(--background)" }}>
            <div className="lg:hidden order-first">
                <DashboardNavbar user={user} role={role} />
            </div>
            <div className="lg:hidden order-2 overflow-x-auto" style={{ borderBottom: "1px solid var(--border)" }}>
            ...
            <div className="flex flex-col flex-1 min-w-0 overflow-hidden order-3 lg:order-0">
                ...
                <main className="flex-1 overflow-y-auto p-4 sm:p-6" style={{ backgroundColor: "var(--background)" }}>
                    {children}
                </main>
            </div>
        </div>
