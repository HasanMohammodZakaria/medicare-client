...
// import { updateDoctorProfile } from "@/app/lib/actions/doctor.action";
...
// const buildForm = (profile, sessionName, sessionImage) => ({
//   doctorName: profile.doctorName ?? sessionName ?? "",
//   specialization: profile.specialization ?? "",
//   qualifications: profile.qualifications ?? "",
//   experience: profile.experience ?? "",
//   consultationFee: profile.consultationFee ?? "",
//   hospitalName: profile.hospitalName ?? "",
//   profileImage: profile.profileImage ?? sessionImage ?? "",
//   phone: profile.phone ?? "",
//   bio: profile.bio ?? "",
// });
...
//   // ✅ initialProfile থেকে form বানাই
//   const [form, setForm] = useState(() =>
//     buildForm(initialProfile, userName, userImage),
//   );

//   // ✅ router.refresh() এর পরে নতুন initialProfile আসলে form sync করি
//   // session load হওয়ার পরেও sync হবে
//   useEffect(() => {
//     setForm(buildForm(initialProfile, userName, userImage));
//   }, [initialProfile, userName, userImage]);
...
export default function DoctorProfile({ initialProfile = {} }) {
  ...
  // ✅ editing না করার সময় সবসময় latest data দেখাবে
  // editing করার সময় user এর input থাকবে
  ...
  // editing mode এ user এর form, otherwise latest server data
  const displayForm = editing ? form : defaultForm;

  const set = (key, val) => setForm((p) => ({ ...p, [key]: val }));

  // editing শুরু হলে current defaultForm দিয়ে form initialize করো
  const startEditing = () => {
    setForm(defaultForm);
    setEditing(true);
  };
  ...
  const handleSave = async () => {
    ...
    setLoading(true);
    try {
      // ✅ doctorId আর পাঠাতে হবে না — action নিজেই getDoctorId() করে
      await updateDoctorProfile(form);
      toast.success("Profile updated successfully!");
      setEditing(false);
      router.refresh(); // ✅ server থেকে নতুন initialProfile আসবে → useEffect → form sync
    } catch {
      toast.error("Failed to update profile. Try again.");
    } finally {
      setLoading(false);
    ...
    <div style={{ padding: "24px 20px" }}>
      ...
      >
        <div style={{ position: "relative", flexShrink: 0 }}>
          ...
          >
            {displayForm.profileImage ? (
              <Image
                src={displayForm.profileImage}
                alt={displayForm.doctorName || "Doctor"}
                width={96}
                height={96}
                style={{ objectFit: "cover", width: "100%", height: "100%" }}
              />
        ...
        <div style={{ flex: 1, minWidth: 200 }}>
          ...
          >
            ...
            >
              {displayForm.doctorName || "Your Name"}
            </h2>
            <span
              style={{
                padding: "3px 10px",
          ...
          >
            {displayForm.specialization || "Specialization not set"}
          </p>
          <div style={{ display: "flex", flexWrap: "wrap", gap: "6px 20px" }}>
            {[
              {
                icon: <MdWork size={14} />,
                text: displayForm.experience
                  ? `${displayForm.experience} years exp.`
                  : "Experience not set",
              },
              {
                icon: <MdLocalHospital size={14} />,
                text: displayForm.hospitalName || "Hospital not set",
              },
              {
                icon: <MdAttachMoney size={14} />,
                text: displayForm.consultationFee
                  ? `$${displayForm.consultationFee} fee`
                  : "Fee not set",
              },
              { icon: <MdEmail size={14} />, text: userEmail || "—" },
            ].map((item, i) => (
        ...
        {editing && (
          ...
          >
            <ImageUploader
              onUpload={(url) => set("profileImage", url)}
              shape="circle"
              defaultImage={displayForm.profileImage}
              label="Profile Photo"
              size="lg"
            />
          </motion.div>
      ...
      >
        ...
        >
          ...
          <FieldRow label="Phone" value={displayForm.phone} editing={editing}>
            <input
              type="tel"
              placeholder="+1 234 567 8900"
              value={form.phone}
          ...
          <FieldRow label="Bio" value={displayForm.bio} editing={editing}>
            <textarea
              placeholder="Brief description about yourself..."
              value={form.bio}
              onChange={(e) => set("bio", e.target.value)}
        ...
        >
          <FieldRow
            label="Specialization"
            value={displayForm.specialization}
            editing={editing}
          >
            <CustomSelect
              value={form.specialization}
          ...
          <FieldRow
            label="Experience (years)"
            value={
              displayForm.experience ? `${displayForm.experience} years` : ""
            }
            editing={editing}
          >
            <input
          ...
          <FieldRow
            label="Hospital / Clinic"
            value={displayForm.hospitalName}
            editing={editing}
          >
            <input
              type="text"
        ...
        >
          <FieldRow
            label="Degrees & Certifications"
            value={displayForm.qualifications}
            editing={editing}
          >
            <textarea
              placeholder={
          ...
          {!editing && displayForm.qualifications && (
            <div
              style={{
                display: "flex",
                flexWrap: "wrap",
        ...
        >
          ...
          {!editing && displayForm.consultationFee && (
            <div
              style={{
                marginTop: -8,
                padding: "16px",
